...
export function parseEditorOptions(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (!name) throw new Error('Unknown or incomplete edit option.');
    if (Object.hasOwn(settings, name)) throw new Error(`Duplicate edit option: ${args[i]}`);
    if (name === 'allowInsecureHttp') { settings[name] = true; continue; }
    if (!args[i + 1] || args[i + 1].startsWith('--')) throw new Error('Unknown or incomplete edit option.');
...
export function editorNetwork(options = {}) {
  if (options.allowInsecureHttp !== undefined && typeof options.allowInsecureHttp !== 'boolean') throw new Error('allowInsecureHttp must be a boolean.');
  ...
  if (allowInsecureHttp && tlsCert) throw new Error('--allow-insecure-http cannot be combined with TLS certificates.');
  const loopback = host === '127.0.0.1' || host === '::1';
  if (!loopback && !tlsCert && !allowInsecureHttp) throw new Error('Non-loopback editor binding requires --tls-cert and --tls-key, or explicit --allow-insecure-http. Use an SSH tunnel for loopback HTTP.');
  let origin;
  if (options.origin !== undefined) {
    ...
    if (url.protocol === 'http:' && !allowInsecureHttp && (!loopback || !['127.0.0.1', '[::1]', 'localhost'].includes(url.hostname))) throw new Error('HTTP browser origins must be loopback; use HTTPS or explicit --allow-insecure-http for hostname access.');
    if (allowInsecureHttp && url.protocol !== 'http:') throw new Error('--allow-insecure-http requires an HTTP origin.');
  ...
  return { host, port, origin, tlsCert, tlsKey, ...(allowInsecureHttp ? { allowInsecureHttp: true } : {}) };
}

export function editorUrl(target, { allowInsecureHttp = false } = {}) {
  ...
  if (url.protocol === 'http:' && allowInsecureHttp !== true && !['127.0.0.1', '[::1]', 'localhost'].includes(url.hostname)) throw new Error('Remote editor URLs require HTTPS or explicit --allow-insecure-http.');
  return url;
}
