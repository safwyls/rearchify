...
import { animateAttr, focusEdgeAttrs, focusNodeAttrs, focusNodeTitle, svgAccessibleText, svgRootAttrs } from '../shared/svg-helpers.mjs';
import { componentBox, boundaryBox, connectionPath } from '../shared/layout-report.mjs';
import { recordDiagnostic, throwDiagnosticProblems } from '../shared/diagnostics-core.mjs';
import { fixedBoundaryMembers } from './boundary-membership.mjs';
...
import { brandLabelFitWidth, brandMetadataFor, brandTopRailProblem, renderBrandMark } from '../shared/brand-rendering.mjs';
...
export function createArchitectureScene(arch) {
...
function boundaryRect(boundary) {
  if (boundary.rect) {
    ...
    return { ...boundary, x, y, width, height, memberTop: y + 24 };
  }
...
function expandBoundaryForReadableTitle(boundary, minimumFontSize) {
  if (boundary.rect || !enforcesBoundaryTitleComposition) return boundary;
...
function layoutBoundaryTitles(rawBoundaries, minimumFontSize) {
  ...
  for (const entry of ordered) {
    ...
    let guard = 0;
    while (!boundary.rect && guard < rawBoundaries.length + components.size + 1) {
      guard += 1;
      const blockers = [
        ...placedTitles,
        ...components.values(),
  ...
  return rawBoundaries.map((_boundary, index) => {
    ...
    const y = enforcesBoundaryTitleComposition && !boundary.rect
...
function validateArchitecture() {
  ...
  for (const [index, boundary] of asArray(arch.boundaries).entries()) {
    ...
    if (boundary.rect) {
      const expected = fixedBoundaryMembers(boundary.rect, components.values());
      const actual = new Set(asArray(boundary.wraps));
      const missing = expected.filter(id => !actual.has(id));
      const outside = [...actual].filter(id => !expected.includes(id));
      if (missing.length || outside.length) {
        const message = `Fixed boundary "${boundary.label}" wraps does not match its rect — set wraps to ${JSON.stringify(expected)} or adjust rect/component positions.`;
        problems.push(message);
        recordDiagnostic({ code: 'layout/boundary-membership', severity: 'error', message,
          subject: { diagramType: 'architecture', collection: 'boundaries', index },
          evidence: { rect: boundary.rect, missing, outside, expected },
          supportedFixes: [`set /boundaries/${index}/wraps to ${JSON.stringify(expected)}`, `adjust /boundaries/${index}/rect to enclose the intended components`],
        });
      }
    }
  }
  ...
  for (const boundary of boundaries) {
    ...
    if (boundary.title.minimumWidth > boundary.title.availableWidth) {
      problems.push(
        ...
        + (boundary.rect ? 'shorten the boundary label or widen rect.' : 'shorten the boundary label, increase pad, or widen the wrapped component layout.'),
      );
    }
...
function alignFacingPorts(conn, from, to, start, end, fromSide, toSide, ports) {
  const hasExplicitGeometry = (
    conn.fromOffset !== undefined || conn.toOffset !== undefined || conn.via
...
function pathFor(conn) {
  ...
  const baseStart = conn.fromOffset !== undefined ? anchor(from, fromSide, conn.fromOffset) : ports?.from || anchor(from, fromSide);
  const baseEnd = conn.toOffset !== undefined ? anchor(to, toSide, conn.toOffset) : ports?.to || anchor(to, toSide);
...
return { renderSvg, validate: validateArchitecture, report: buildLayoutReport,
  // Editing needs exact values; the public CLI report deliberately rounds its
  // coordinates for readability. Return copies so drags cannot mutate a scene.
  editGeometry: () => ({
    components: [...components.values()].map(c => ({ ...c })),
    connections: asArray(arch.connections).map(conn => ({
      points: pathFor(conn).points.map(point => [...point]),
      labelAt: conn.label ? [...labelPoint(conn, pathFor(conn).points)] : null,
    })),
  }),
};
}
